...
class Payment(UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    gateway = models.CharField(_('payment gateway'), max_length=50, null=True, blank=True, help_text=_('e.g., zarinpal, pasargad'))
    transaction_id = models.CharField(_('transaction ID'), max_length=100, null=True, blank=True, unique=True, db_index=True)
    ...
    def __str__(self):
        return f"Payment {self.id} for Order #{self.order.order_number} - {self.get_status_display()}"