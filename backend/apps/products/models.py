...
class Product(UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    short_description = models.CharField(
        _("short description"), max_length=500, null=True, blank=True
    )
    ...
    product_type = models.CharField(
        _("product type"), max_length=20, choices=ProductType.choices, default=ProductType.SIMPLE
    )
    is_active = models.BooleanField(_("is active"), default=True)
...
class ProductImage(UUIDPrimaryKeyMixin):
    ...
    def __str__(self):
        ...
        return f"Image for {self.product.name} ({status})"
...
class ProductAttributeValue(UUIDPrimaryKeyMixin):
    ...
    value_number = models.DecimalField(
        _("number value"), max_digits=10, decimal_places=2, null=True, blank=True
    )
