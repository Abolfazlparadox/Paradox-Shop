...
class CategoryAttribute(UUIDPrimaryKeyMixin):
    ...
    category = models.ForeignKey(
        Category, on_delete=models.CASCADE, related_name="attributes", verbose_name=_("category")
    )
    ...
    is_filterable = models.BooleanField(
        _("is filterable"), default=False, help_text=_("Used for search/catalog filters")
    )
    is_variant = models.BooleanField(
        _("is variant"), default=False, help_text=_("Used to define product SKUs/variants")
    )
    sort_order = models.IntegerField(_("sort order"), default=0)
