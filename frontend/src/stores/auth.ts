...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  setTokens: (tokens: TokenPair) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      const secureSuffix = getCookieSecuritySuffix();
      document.cookie = `pdx_auth_token=${tokens.access}; path=/; max-age=604800; SameSite=Lax${secureSuffix}`;
    }
    set({
  ...
  clearAuth: () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      const secureSuffix = getCookieSecuritySuffix();
      document.cookie = `pdx_auth_token=; path=/; max-age=0; SameSite=Lax${secureSuffix}`;
      document.cookie = `pdx_is_staff=; path=/; max-age=0; SameSite=Lax${secureSuffix}`;
    }
    set({
      user: null,
      accessToken: null,
  ...
  login: async (credentials: LoginRequest) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && (profile.is_staff || profile.is_superuser)) {
        const secureSuffix = getCookieSecuritySuffix();
        document.cookie = `pdx_is_staff=true; path=/; max-age=604800; SameSite=Lax${secureSuffix}`;
      }
